...
public class MyBot : IChessBot
{
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        {   
            ...
            double currentEval = MaxMin(board, 1, double.MinValue, double.MaxValue, false);
    ...
    double MaxMin(Board board, int depth, double alpha, double beta, bool maxPlayer)
    {
        ...
        {
            return EvalPos(board, maxPlayer);
        }
...
//	None = 0, Pawn = 1, Knight = 2, Bishop = 3, Rook = 4, Queen = 5, King = 6
    ...
    double EvalPos(Board board, bool maxPLayer)
    {
        double currentEval = 0;
           
        if (board.IsInCheckmate())
        {
            return !maxPLayer? double.MaxValue : double.MinValue;
        }
        ...
        foreach (PieceList pieceList in AllPieces)
        {
            ...
            int AddVal = pieceValues[ typeAsInt ] * pieceList.Count;
            ...
            for(uint i=0; i<4; i++)
            {
                    ulong MaskedBoard = PieceBonus[typeAsInt-1, i] & PieceBitBoard;
                    piecesPositionBonus += BitboardHelper.GetNumberOfSetBits(MaskedBoard) * 50;
            }

            if (typeAsInt == 3 || typeAsInt == 4 || typeAsInt == 5)
            {
